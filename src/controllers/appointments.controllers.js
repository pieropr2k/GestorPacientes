...
import MedicalNotesModel from '../models/medical_notes.model.js';
    ...
    try {
        ...
        if (state === 'canceled' || state === 'no show') {
            ...
            if (medical_history_info) {
                medical_history_id = await MedicalHistoryModel.updateMedicalHistory(medical_history_info.id,
                    {diagnosis, treatment, notes}
                )
            } else {
                medical_history_id = await MedicalHistoryModel.createMedicalHistory({
                doctor_id: existingDoctorUser.id, patient_id: "020459789", diagnosis, treatment, appointment_id: appointmentId, notes
            })
            }
            
            // borrar notas
            /*
            notes.forEach(note => MedicalNotesModel.createMedicalNote({
                description: note,
                medical_history_id: medical_history_id
            }))
            */
            
            //console.log({doctor_id: existingAppointment.doctor_id, client_id, diagnosis, treatment, notes})
        } else {
